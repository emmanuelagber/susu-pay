    ...
    >
        ...
        className={[
          ...
          isDone
            ...
            : isFailed
              ...
              : isCurrent
                ? "bg-blue-accent/20 text-blue-accent ring-2 ring-blue-accent/40"
      ...
      <div className="flex-1 min-w-0">
        <p
          className={[
            "text-sm font-medium",
            isCurrent ? "text-text-base" : "text-text-dim",
          ].join(" ")}
        >
          {entry.name}
        </p>
        <p className="text-xs text-text-ghost font-mono">
          {entry.virtualAccount}
        </p>
      </div>
      <div className="text-right flex-shrink-0">
        <p className="text-sm font-semibold text-text-base tabular-nums">
          {fmt(entry.payoutAmount)}
        </p>
        {isCurrent && (
        ...
        {isDone && (
          <Badge variant="green" dot>
            Paid {entry.paidDate ? fmtDate(entry.paidDate) : ""}
          </Badge>
        )}
        {isFailed && (
...
function HistoryRow({ record }: { record: PayoutHistoryRecord }) {
  return (
    <tr className="border-b border-border last:border-0 hover:bg-surface-alt/40 transition-colors">
      ...
      <td className="py-3 px-4 text-sm text-text-dim text-center">
      ...
      <td className="py-3 pr-5 pl-4 text-xs text-text-ghost">
        {fmtDate(record.processedAt)}
      </td>
    </tr>
  );
}
...
export default function PayoutFlow() {
  ...
  const { circleId, selectedCircle, setCircleId, circles } =
    useSelectedCircle();
          ...
          <div className="flex items-center gap-2 mt-1.5">
            ...
            {info && (
              <span className="text-xs text-[#ffffff] font-medium">
                Cycle {info.cycleNumber} of {info.totalCycles} cycles
              </span>
            )}
          </div>
        </div>
        ...
        <>
          ...
          >
            ...
            {info.currentRecipient ? (
                ...
                <div className="flex-1 flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-base font-semibold text-text-base">
                      {info.currentRecipient.name}
                    </p>
                    <p className="text-xs text-text-ghost">
                      Position #{info.currentRecipient.position} ·{" "}
                      {info.currentRecipient.virtualAccount}
                    </p>
                  </div>
                </div>
              ...
              <p className="text-sm text-text-ghost mb-5">
            ...
            {!info.canTrigger && info.blockers.length > 0 && (
              <div className="bg-amber-accent/10 border border-amber-accent/25 rounded-lg px-3.5 py-2.5 mb-4">
                {info.blockers.map((b, i) => (
                  <p key={i} className="text-xs text-amber-accent">
                    {b}
                  </p>
                ))}
              </div>
            ...
            >
              ...
              {payoutMutation.isSuccess
                ? "Payout processing…"
                : `Trigger payout — ${fmt(info.expectedPayoutAmount)}`}
            </Button>
          </div>
            ...
            <tbody>
              {histLoading ? (
                <tr>
                  <td
                    colSpan={5}
                    className="py-10 text-center text-text-ghost text-sm"
                  >
                    Loading…
                  </td>
                </tr>
